...
class MCMC:
	...
	def class_perf(self, actual_list, prediction_list):  

		#print(actual_list, ' is actual list')

		#print(prediction_list, ' is prediction_list')

		tolerance = 0.3
		num_instances = actual_list.shape[0]
		class_perf = 0 

		for s in range(0, num_instances):	

			pred_binary = np.zeros(prediction_list.shape[1])
			actual  = actual_list[s,:] 
			prediction = prediction_list[s] 
			index= np.argmax(prediction)
			pred_binary[index] = 1
			#print(prediction, pred_binary, actual, ' *')
			#pred_binary = np.where(prediction > (1 - tolerance), 1, 0) # this was applicable when we used sigmoid in output 
			if( (actual==pred_binary).all()):
				class_perf =  class_perf +1   
	...
	def likelihood_func(self, model, data, w, tausq):
		...
		#print(y[0,:], fx[0,:], '   ***')
		if (self.regression is True): # Gaussian liklihood
			accuracy = self.rmse(fx, y) #RMSE 
			loss = np.sum(-0.5*np.log(2*math.pi*tausq) - 0.5*np.square(y-fx)/tausq)
			#print(loss, '  is loss') 
		else:
			accuracy = self.class_perf(y, fx) # Classification performance
			
			loss = 0   # Multinomial likelihood
			z = np.zeros((data.shape[0],self.topology[1]))
	 
			for i in range(data.shape[0]):
				for j in range(self.topology[1]):
					if j == y[i,j]:
						z[i,j] = 1
					loss += z[i,j]*np.log(fx[i,j])
